...
Every production database eventually needs to jump major versions (13 → 14 → 15 → 16). You have two strategies, with a huge trade-off between safety and downtime.
...
```bash
# On a fresh machine with the new Postgres version installed:
pg_dumpall -h old_host -U postgres > full_backup.sql
psql -h new_host -U postgres -f full_backup.sql
```

**Pros:** Always works. Tests restore path. Leaves old cluster untouched as a rollback.
...
`pg_upgrade` does an **in-place** upgrade by reusing the existing data files. It's orders of magnitude faster — minutes instead of hours — because it doesn't move user data. It only rewrites system catalogs.
...
# 2. Initialize the new cluster (empty data directory)
/usr/lib/postgresql/16/bin/initdb -D /var/lib/postgresql/16/main

# 3. Stop BOTH clusters (critical — data must be quiescent)
...
# 7. Run the analyze script pg_upgrade generated
./analyze_new_cluster.sh

# 8. Delete the old cluster once confirmed working
...
| Mode | Speed | Safety |
|------|-------|--------|
| `--copy` (default) | Slow (copies all files) | Old cluster remains as rollback |
| `--link` | Instant | **Old cluster is unusable after upgrade** — files are shared. No rollback. |
| `--clone` (btrfs/xfs) | Near-instant | Old cluster usable briefly (until first write) |

**If you use `--link`, take a full backup first.** Once you start the new cluster, the old data directory is corrupted (the new version writes to the shared files in a new format).

### What pg_upgrade Cannot Do

- **Extensions must match** between old and new versions. Install the extension packages on the new cluster before upgrading.
- **Default collations** — if the OS upgraded `glibc` and collation order changed, text indexes may be corrupt. Reindex text indexes after upgrade:

```sql
REINDEX INDEX CONCURRENTLY idx_name;
-- Or everything in one shot (locks the table):
REINDEX TABLE CONCURRENTLY anomaly_routes;
```

- **Cannot downgrade.** If the new version has a problem, you restore from backup.

### Statistics Are Not Carried Over

`pg_upgrade` does NOT copy optimizer statistics. Immediately after upgrade, every query has no statistics → catastrophic plans. You must run:

```bash
./analyze_new_cluster.sh  # generated by pg_upgrade
# or manually:
vacuumdb --all --analyze-in-stages
# --analyze-in-stages runs ANALYZE 3 times with increasing accuracy — first pass is fast,
# giving the planner rough statistics immediately; later passes refine them.
```
...
1. Set up the new cluster as a logical replica of the old cluster
2. Wait for initial sync + streaming catch-up
3. At cutover: point application to new cluster, stop old cluster
...
|----------|---------|-------------------|
| `Lock` | Waiting for a heavyweight lock (row/table) | Another transaction holds the lock |
| `LWLock` | Waiting for an internal lightweight lock | Contention on a shared data structure |
| `BufferPin` | Waiting to pin a buffer for exclusive access | VACUUM or another backend has it |
| `IO` | Waiting on disk I/O | Slow storage, cold cache, big sort spill |
| `IPC` | Waiting on inter-process communication | Parallel workers, replication |
| `Timeout` | Waiting on a timer | `pg_sleep`, deadlock detection |
| `Extension` | Waiting inside an extension's code | Extension-specific |
| `Client` | Waiting for the client to send/receive data | Slow app, small buffer, network |
| `Activity` | Backend is idle, waiting for work | Normal — background processes |
...
**`LWLock: WALWrite`** — backends piling up waiting for WAL to flush to disk. Usually means:
- `synchronous_commit = on` + slow disk (fsync bottleneck)
- `wal_buffers` too small
- Too many tiny transactions — batch them

**`LWLock: LockManager`** — heavy contention on the internal lock manager. Usually means you have a query acquiring locks on thousands of partitions at once. Fix: partition pruning, fewer partitions in a single query.

**`LWLock: BufferContent`** — multiple backends trying to modify the same page. Hot row problem. Often a counter table everyone updates. Fix: partition the counter, use `SKIP LOCKED`, or aggregate writes.
...
**`IO: DataFileWrite`** — waiting for dirty page flush. Usually means `checkpoint_completion_target` too aggressive or checkpoints too close together.

**`IO: WALSync`** — waiting for `fdatasync` on WAL. Disk is the bottleneck. Options: faster storage, `synchronous_commit = off` (unsafe), group commits.
...
**`Client: ClientRead`** — the query is done, waiting for the app to read results or send the next command. If this dominates your `idle in transaction` connections: **your app is slow, not Postgres.** Check pool configuration, result set size, network latency.

### Historical Wait Event Sampling

`pg_stat_activity` is a single point-in-time snapshot. For production diagnosis you need **history** — what events dominated over the last hour. The standard tool is `pg_wait_sampling`:

```sql
-- Extension that samples wait events at high frequency:
CREATE EXTENSION pg_wait_sampling;

-- Top wait events across all queries in the last sample window:
SELECT event_type, event, count
FROM pg_wait_sampling_profile
ORDER BY count DESC
LIMIT 20;

-- Wait events per query (requires pg_stat_statements joined by queryid):
SELECT ss.query,
       ws.event_type,
       ws.event,
       ws.count
...
Run this every second during a slow period to see what's actually happening:

```sql
-- What's everyone waiting on right now — one-liner:
